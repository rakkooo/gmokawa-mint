/* -------- external ESM import -------- */
import * as KuruSdk from "https://esm.sh/@kuru-labs/kuru-sdk@0.0.46";   // ✔ESM CDN :contentReference[oaicite:2]{index=2}
...
/* Build swap TX with KuruSdk (browser ESM) */
async function buildMarketTx(size="1"){
  ...
  const params=await KuruSdk.ParamFetcher.getMarketParams(p,MARKET);               // :contentReference[oaicite:3]{index=3}
...
connectBtn.onclick=async()=>{
  ...
  if(parseInt(id,16)!==CHAIN){
    await ethereum.request({                       // MetaMask chain API
      method:"wallet_addEthereumChain",
      params:[{chainId:HEX,chainName:"Monad Testnet",rpcUrls:[RPC],
